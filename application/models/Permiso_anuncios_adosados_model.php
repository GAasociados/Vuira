...
//        $this->db->where('autorizacion', 1);
    ...
    function get_reporte($fechainicio,$fechafinal,$nombrepropietario,$status)
    {
      ...
      if (!empty($fechainicio)) {
        ...
        $FechaINFull="SELECT * FROM ".$this->table." WHERE fechainiciocompleta is not NULL AND fechainiciocompleta >= '".$fechainicio."'";
        $FIF=$this->db->query($FechaINFull);

        if($FIF->result())
        {
            $SQL .="fechainiciocompleta >= '".$fechainicio."' AND ";
        }
        else
        {
      ...
      if (!empty($fechafinal)) {
        ...
        $FechaFiFull="SELECT * FROM ".$this->table." WHERE fechafinalcompleta is not NULL AND fechafinalcompleta >= '".$fechafinal."'";
        $FFF=$this->db->query($FechaFiFull);

        if($FFF->result())
        {
            $SQL .="fechafinalcompleta >= '".$fechafinal."' AND ";
        }
        else
        {
            $SQL .="fechafinal >= '".$fechafinal."' AND ";
