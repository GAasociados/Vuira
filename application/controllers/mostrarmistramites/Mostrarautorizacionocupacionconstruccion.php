...
class Mostrarautorizacionocupacionconstruccion extends CI_Controller {
	...
	public function mostrartram($id = null){
		...
		if ($this->session->userdata("login") == false ){//|| $this->session->userdata("tipo") != 3) {
			header("Location:".base_url()."home");
		}
		else{
		$datos = $this->Autorizacionocupacionconstruccion_model->mostrarByIdFuncionario($id);
		...
		if ((empty($datos))) {
			...
			$this->load->view("mostrarmistramites/editarautorizacionocupacionconstruccionfuncionario",$data);
