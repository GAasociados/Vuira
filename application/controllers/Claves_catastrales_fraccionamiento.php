...
class Claves_catastrales_fraccionamiento extends CI_Controller {
    ...
    public function pago($id = null) {
        if ($id) {
            ...
            if ($row && $row->usuarioID === $this->session->userdata('id') && $row->status === "4") {
                ...
                $data['ref'] = "002";
                $data['id'] = "claves_catastrales_fraccionamiento/update/" . $id;
...
//       die(print_r(   fclose($fp1)));
    ...
    public function index() {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'claves_catastrales_fraccionamiento/?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'claves_catastrales_fraccionamiento/?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'claves_catastrales_fraccionamiento/';
            $config['first_url'] = base_url() . 'claves_catastrales_fraccionamiento/';
        }
        ...
        if ($this->session->userdata('tipo') == 4 || $this->session->userdata('tipo') == 3) {
            $config['total_rows'] = $this->Claves_catastrales_fraccionamiento_model->total_rows_ventana($q);

            $claves_catastrales_fraccionamiento = $this->Claves_catastrales_fraccionamiento_model->get_limit_data_ventana($config['per_page'], $start, $q);
        } else {

            $config['total_rows'] = $this->Claves_catastrales_fraccionamiento_model->total_rows($q);
            $claves_catastrales_fraccionamiento = $this->Claves_catastrales_fraccionamiento_model->get_limit_data($config['per_page'], $start, $q);
        }
        ...
        $this->load->view('claves_catastrales_fraccionamiento/claves_catastrales_fraccionamiento_list', $data);
    }

    public function reportes() {
        $this->load->view('claves_catastrales_fraccionamiento/claves_catastrales_fraccionamiento_reportes');
    }
...
//  //download it.
    ...
    public function create() {
        ...
        $data = array(
            ...
            'action' => site_url('claves_catastrales_fraccionamiento/create_action'),
            'ID' => set_value('ID'),
    ...
    public function create_ventanilla() {
        ...
        $noficial = $this->db->query("SELECT `id`,`tipo_documento`,`archivo`,`fecha` FROM `documentos_ciudadano` WHERE `tipo_documento` = 'Numero Oficial' AND  `usuarios_ID` = '$user_id' ");
        $data['noficial'] = $noficial;
        $this->load->view('claves_catastrales_fraccionamiento/claves_catastrales_fraccionamiento_form_fun', $data);
    }

    public function create_action() {
        $maxnum = $this->Claves_catastrales_fraccionamiento_model->getMaxItemByid();
...
//        die(print_r($data,true));
        $this->Claves_catastrales_fraccionamiento_model->insert($data);
        $this->session->set_flashdata('correcto', 'Su solicitud de Clave Catastral Fraccionamiento ha sido registrada con éxito, un Funcionario dará revisión y le notificara vía correo electrónico');
        if ($this->session->userdata('tipo') == 15) {
            redirect(site_url('claves_catastrales_fraccionamiento/update/' . $maxnumbd));
        } else {
            redirect(site_url('claves_catastrales_fraccionamiento'));
        }
    }
    ...
    public function update($id) {
        ...
        $costo = $this->db->query("SELECT * FROM `costos_tramite` WHERE `tipo` IN (1,2) ");
        $row2 = $this->Claves_fraccionamiento_model->get_by_id_administrador($row->id_solicitud);
//         die(print_r($row2));
        if ($row) {
            $data = array(
                'consulta' => $result,
                ...
                'telefonodp' => set_value('telefonodp', $row->telefonodp),
                'tipotramitedp' => set_value('tipotramitedp', $row->tipotramitedp),
                'doctoine' => set_value('doctoine', $row2->doctoine),
                'doctopodersimple' => set_value('doctopodersimple', $row2->doctopodersimple),
                'doctoplanodigital' => set_value('doctoplanodigital', $row2->doctoplanodigital),
                'doctolegal' => set_value('doctolegal', $row2->doctolegal),
                'doctooficio' => set_value('doctooficio', $row2->doctooficio),
                'doctoplanofisico' => set_value('doctoplanofisico', $row2->doctoplanofisico),
                'doctopredial' => set_value('doctopredial', $row2->doctopredial),
                'doctopago' => set_value('doctopago', $row->doctopago),
                'doctofinal' => set_value('doctofinal', $row->doctofinal),
                'status' => set_value('status', $row->status),
                'mensaje' => set_value('mensaje', $row->mensaje),
                ...
                'cantidad' => set_value('cantidad', $row->cantidad),
                'costo' => $costo->result(),
                'morine' => set_value('morine', $row2->morine),
                'poder_nota' => set_value('poder_nota', $row2->poder_nota),
                'fisrfc' => set_value('fisrfc', $row2->fisrfc),
                'acta_const' => set_value('acta_const', $row2->acta_const),
                'coloniados' => set_value('acta_const', $row2->coloniados),
                'fechafinal' => set_value('fechafinal', $row2->fechafinal),
                'valido_pago' => set_value('valido_pago', $row2->valido_pago),
            );
    ...
    public function asignar_func() {
        ...
        foreach ($id as $i) {
            ...
            $row2 = $this->Claves_catastrales_fraccionamiento_model->get_all_reg($row->id_solicitud);
            $flag = true;
...
//           
            ...
            if ($flag == true) {
                ...
                $this->Claves_fraccionamiento_model->update($row->id_solicitud, $data);
            }
    ...
    public function update_action_clave() {
        if ($this->form_validation->run() == TRUE) {
            ...
            $config1['upload_path'] = './assets/tramites/clavescatastralesfraccionamiento/';
...
//                    $data['notificacion'] = 0;
            ...
            if (!empty($_FILES['doctofinal']['tmp_name'])) {
                if ($row->doctofinal) {
                    if (is_readable("./assets/tramites/clavescatastralesfraccionamiento/" . $row->doctofinal)) {
                        unlink("./assets/tramites/clavescatastralesfraccionamiento/" . $row->doctofinal);
                    }
                }
                $ext = "";
            ...
            $this->Claves_catastrales_fraccionamiento_model->update($this->input->post('ID', TRUE), $data);
            $row = $this->Claves_catastrales_fraccionamiento_model->get_by_id_administrador($this->input->post('ID', TRUE));
            $row2 = $this->Claves_catastrales_fraccionamiento_model->get_all_reg($row->id_solicitud);
            $flag = true;
...
//                'numeronotificacionint' => set_value('numeronotificacionint', $row->numeronotificacionint),
            ...
            $data['action'] = site_url('Claves_catastrales_fraccionamiento/vistapago_action');
            ...
            $this->load->view('Claves_catastrales_fraccionamiento/Claves_catastrales_fraccionamiento_vistapago', $data);
        } else {
            //$this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('Claves_catastrales_fraccionamiento'));
        }
    ...
    public function vistapago_action() {
        ...
        if ($this->input->post()) {
            if ($_FILES['compro']['tmp_name'][0]) {
                ...
                redirect(site_url('Claves_catastrales_fraccionamiento/vistapago/' . $this->input->post('ID', TRUE)));
            }
        }
    }
...
//           
    ...
    public function ventanilla() {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'claves_catastrales_fraccionamiento/ventanilla/?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'claves_catastrales_fraccionamiento/ventanilla/?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'claves_catastrales_fraccionamiento/ventanilla/';
            $config['first_url'] = base_url() . 'claves_catastrales_fraccionamiento/ventanilla/';
        }
        ...
        $claves_catastrales_individual = $this->Claves_catastrales_fraccionamiento_model->get_limit_data_ventana($config['per_page'], $start, $q);
...
//        die()
        ...
        $this->load->view('claves_catastrales_fraccionamiento/claves_catastrales_fraccionamiento_list_ventanilla', $data);
    }
