                ...
                <tr>
                    <td align="justify" style="font-size: 14px;">
                        ...
                        ?>  " type="hidden">
                                                              ...
                                                              ?> del inmueble ubicado en la calle <b><input name="calle" value="<?php echo $calle ?>" type="hidden"> <?php echo $calle; ?></b> con número oficial <b><input name="numero" value="<?php echo $numero; ?>" type="hidden"> <?php echo $numero; echo $noloteui !=""? ", ".$noloteui:"" ; echo $nomanzanaui !=""? ", ".$nomanzanaui:""; ?>  </b> del Fracc./Colonia/Ejido/Comunidad/Barrio<b><input name="colonia" value="<?php if($colonia=='Colonia NO asignada')echo $coloniados; else echo $colonia; ?>" type="hidden"> <?php if($colonia=='Colonia NO asignada')echo $coloniados; else echo $colonia; ?></b> de esta ciudad de Irapuato Guanajuato, cuya superficie de terreno es de <b><input name="sup" value="<?php echo $sup ?>" type="hidden"> <?php echo $sup; ?> </b> registrado bajo la cuenta predial <?php if ($predial != 'APERTURA'): ?>
                             <b><input name="predial" value="<?php echo $predial; ?>" type="hidden"> <?php echo $predial; ?></b>
<?php else: ?><input name="predial" value="sin número de cuenta predial" type="hidden">
                            <b>S/N</b>
 <?php endif; ?>se le informa que una vez consultada la base de datos de esta Dirección y la base de datos del padrón de contribuyentes de la Dirección de Impuestos Inmobiliarios, así como la revisión realizada a la documentación que nos anexa se concluye que: 
                        <br>
                        <br>
                        El bien inmueble señalado se encuentra inscrito a nombre de <b> INSTITUTO MUNICIPAL DE VIVIENDA DE IRAPUATO (IMUVII), </b>con el carácter de <b><input name="caracter" value="PROPIETARIO" type="hidden">PROPIETARIO</b>, <?php if ($coop): ?>
                            y CO-PROPIETARIO <?php echo $coop; ?> <input name="coop" value="<?php echo $coop; ?>" type="hidden"> 
                        <?php endif; ?> y en atención a <b><input name="d2" value="<?php echo $d2; ?>" type="hidden"> <?php ECHO $d2; ?> </b> sustentando mediante la <b>CONSTANCIA IMUVII/DG/UR/01 <input type="hidden" name="constancia" value="IMUVII/DG/UR/01"></b> con <b>NÚMERO DE FOLIO <?php echo $no_folio;?><input type="hidden" name="no_folio" value="<?php echo $no_folio;?>"></b>, con fecha del <?php
                        setlocale(LC_ALL, "es_ES@euro", "es_ES", "esp");
                        $fecha = strftime("%d de %B de %Y", strtotime($fecha_const));
                        echo $fecha;
                        ?><input name="fecha_esc" value="<?php echo $fecha; ?>" type="hidden">, debidamente sellada y rubricada por <b><?php echo $nombre_imuvii;?><input value="<?php echo $nombre_imuvii;?>" name="nombre_imuvii" type="hidden" ></b> Directora del Despacho de la Dirección Jurídica.<br><br>
  ...
  <br>       <br>
            ...
            <button type="button"  onclick="cerrar()"> Regresar a trámite </button>
        </form>
    </body>
